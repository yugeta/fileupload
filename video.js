;$$fileupload_video = (function(){
  ...
  // [初期設定] テンプレートhtmlをセット
  SET.prototype.setTemp_video = function(main){
    var lib  = new LIB();
    ...
    onSuccess : (function(main,res){
      main.options.video_template = res;
    }).bind(this,main)
    });
  };
  ...
  SET.prototype.setList = function(main , fl,i){
    ...
    var doc = parser.parseFromString(main.options.video_template, "text/html");
    var li = doc.querySelector("."+main.options.dom.li);
    li.setAttribute("data-num" , i);

    var video  = li.querySelector("."+main.options.dom.video);
    var source = video.querySelector("."+main.options.dom.source);

    var path = URL.createObjectURL(fl);
    source.src = path;

    video.setAttribute("data-num"    , i);
    video.setAttribute("data-type"   , fl.type);
    video.setAttribute("data-size"   , fl.size);
    ...
    if(info_time){
      lib.event(video , "loadedmetadata" , (function(info_time,e){
        var video = e.target;
        var tm = video.duration;
        info_time.textContent = new LIB().setFormatTime(tm);
        video.setAttribute("data-time" , video.duration);
      }).bind(this , info_time));
    }
    ...
    reader.onload = (function(main , li , filename , type , size , e){
      ...
      var endtime = li.querySelector("."+main.options.dom.video).getAttribute("data-time");
      if(tag_area){
        var htmls = [];
...
//   };
  ...
  POST.prototype.postFile = function(main , viewListElement){
    ...
    // areaに送信中フラグをセット
    var area = viewListElement.querySelector("."+main.options.dom.video_area);
    if(area){
    ...
    fd.append("videoFile"    , post.postFiles_cache[0]);
    ...
    var video = viewListElement.querySelector("."+ main.options.dom.video);
    fd.append("info[type]"   , post.set_postStringFormat(main , video.getAttribute("data-type")));
    fd.append("info[size]"   , post.set_postStringFormat(main , video.getAttribute("data-size")));
    fd.append("info[time]"   , post.set_postStringFormat(main , video.getAttribute("data-time")));
